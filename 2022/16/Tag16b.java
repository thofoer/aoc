...
public class Tag16b {
    ...
    static private int process(String pos, int minLeft, long openValves) {
        long key = (long) (posIds.get(pos) << 25L) + ((long) minLeft << 17L) + openValves;
    ...
    public static void main(String[] args) throws Exception {
        ...
        var allValvesMask = (1 << posIds.size()) -1 ;

        var best = 0;
        for (int i=0; i< allValvesMask >> 1; i++) {
            best = Math.max(best, process("AA", 26, i) + process("AA", 26, i ^ allValvesMask));
        }

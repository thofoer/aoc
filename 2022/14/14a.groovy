...
maxWidth = vals.withIndex().findAll {v,i -> (i & 1)==0}.collect{it[0]}.max() + 1
maxHeight = vals.withIndex().findAll {v,i -> (i & 1)==1}.collect{it[0]}.max() + 1
...
def dump() {
    matrix.each { println it.join("").substring(0) }
    println ""
}

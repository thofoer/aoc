...
input.each.with_index do |l, y|
    l.each_char.with_index do |c,x|
        grid << Vector[x,y,0,0] if c == ?#
    end
end

OFFSETS = (-1..1).map{|x| (-1..1).map{|y| (-1..1).map{|z| (-1..1).map{|w| Vector[x,y,z,w]}}}}.flatten(3)
OFFSETS.delete(Vector[0,0,0,0])
...
def step(grid)
    ...
    @wrange = @wrange.min-1..@wrange.max+1

    coords = @xrange.map{|x| @yrange.map{|y| @zrange.map{|z| @wrange.map{|w| Vector[x,y,z,w]}}}}.flatten(3)
    newGrid = Set.new
